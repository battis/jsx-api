...
import Routing, { PageHandler } from "@battis/jsx-routing";
    ...
    return (
      <form method="post" action={this.authorize_uri}>
        ...
        {Object.getOwnPropertyNames(this.request).map((key) => (
          <input name={key} type="hidden" value={this.request[key]} />
        ))}
        ...
        <div class="buttons">
          <button class="default" type="submit" name="authorized" value="yes">
            Login
          </button>
          <button type="button" onclick={() => Routing.redirectTo("/")}>
            Cancel
          </button>
        </div>
      </form>
...
export default class Authentication extends RequireInitialization {
  ...
  private static config: AuthenticationConfig = {
    ...
    logoutHandler: () => Routing.navigateTo(Authentication.config.login_path),
  };
  ...
  public static init(config: Partial<AuthenticationConfig> = {}) {
    ...
    Routing.add(Authentication.login_path, Authentication.userLogin)
      .add(Authentication.logout_path, Authentication.userLogout)
      .add(
        Authentication.oauth2_authorize_path,
        Authentication.handleAuthorization
      )
      .add(
        Authentication.oauth2_redirect_path,
        Authentication.handleOAuth2Redirect
      )
      .add(
        Authentication.oauth2_authorized_path,
        Authentication.handleAuthorized
      );
  ...
  public static userLogin: PageHandler = () => {
    if (!Authentication.access_token) {
      this.displaySpinner();
      Authentication.oauth_state = uuid();
      ...
      Authentication.clientDescriptor(OAuth2.client_id).then((client) => {
        render(
          <Authentication.Login
            client={client}
            request={new AuthorizationRequest(Authentication.oauth_state)}
  ...
  private static async handleAuthorization() {
    ...
    Authentication._client = await Authentication.clientDescriptor(
      request.client_id
    );
    render(
      <Authentication.Login
  ...
  private static handleAuthorized() {
    render(
      <div>
        <h1>
          {`${
            Authentication._client?.display_name || "Application"
          } Authorized`}{" "}
          closeable={false}
        </h1>
            ...
            await API.post(
              {
                ...
                body: new AuthorizationCodeGrantRequest(authorization),
              },
              false
            )
          )
